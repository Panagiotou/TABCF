...
class Model:
    ...
    def __init__(self, model=None, model_path='', backend=BackEndTypes.Tensorflow1, model_type=ModelTypes.Classifier,
                 func=None, kw_args=None, device="cpu"):
        """Init method
        ...
        if model is None and model_path == '':
            ...
            self.decide_implementation_type(model, model_path, backend, func, kw_args, device)

    def decide_implementation_type(self, model, model_path, backend, func, kw_args, device):
        """Decides the Model implementation type."""

        self.__class__ = decide(backend)
        self.__init__(model, model_path, backend, func, kw_args, device)

...
Normalization = Literal['min_max_torch', 'standard_torch', 'standard', 'quantile', 'minmax']
NumNanPolicy = Literal['drop-rows', 'mean']
CatNanPolicy = Literal['most_frequent']
CatEncoding = Literal['one-hot', 'one-hot-drop-bin', 'counter']
...
class TorchMinMaxScaler:
    ...
    def fit(self, x):
        x = self.check_make_tensor(x)
        self.data_min = torch.min(x, dim=0, keepdim=True).values
        self.data_max = torch.max(x, dim=0, keepdim=True).values
        self.min = self.feature_range[0]
        self.max = self.feature_range[1]
    
    def transform(self, x):
        x = self.check_make_tensor(x)
        x_std = (x - self.data_min) / (self.data_max - self.data_min + 1e-7)
    ...
    def inverse_transform(self, x):
        x = self.check_make_tensor(x)
        device = x.device
        x_std = (x - self.min) / (self.max - self.min + 1e-7)
        x_original = x_std * (self.data_max.to(device) - self.data_min.to(device)) + self.data_min.to(device)
        return x_original
    
    def check_make_tensor(self, x):
...
class TorchStandardScaler:
    ...
    def inverse_transform(self, x):
        x = self.check_make_tensor(x)
        device = x.device
        x = x * (self.std.to(device) + 1e-7) + self.mean.to(device)
        return x
...
def normalize(
    ...
    if normalization == 'standard':
        ...
        #         X_train.shape
        #     )
    elif normalization== 'standard_torch':
        normalizer = TorchStandardScaler()
    elif normalization== 'min_max_torch':
        normalizer = TorchMinMaxScaler()
    else:
...
def cat_encode(
    ...
    elif encoding == 'one-hot-drop-bin':
        ohe = sklearn.preprocessing.OneHotEncoder(
            handle_unknown='ignore', drop="if_binary", sparse_output=False, dtype=np.float32 # type: ignore[code]
        )
        encoder = make_pipeline(ohe)
